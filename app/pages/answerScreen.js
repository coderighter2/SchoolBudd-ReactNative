...
export default class AnswerScreen extends React.Component {
  ...
  restoreVote = async () => {
    await firebase
      .database()
      .ref("forum")
      .child(this.state.forumLocation)
      .child("answers")
  ...
  render() {
    return (
      <View style={styles.container}>
        <View>
          <Text style={{ fontSize: 28, width: "100%", fontWeight: "bold" }}>
            {this.state.question}
          </Text>
          <Text style={{ fontSize: 13, color: "#595959", marginTop: 10 }}>
        ...
        <View style={{ marginTop: 15 }}>
          <Text style={styles.cardText}>{this.state.answer}</Text>
        </View>
